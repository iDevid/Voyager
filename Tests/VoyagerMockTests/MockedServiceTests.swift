...
class MockedServiceTests: XCTestCase {
    ...
    var service: TestServiceMock!
    
    override func setUp() {
        super.setUp()
        service = TestServiceMock()
    }
    
    func testInjectability() {
        let exp = XCTestExpectation()
        service.setNextResponse(.json(name: "test_1", bundle: Bundle.module))
        injectionMethod(injectedService: service, expectation: exp)
        wait(for: [exp], timeout: 2)
    }
    
    private func injectionMethod(injectedService: some TestServiceInterface, expectation: XCTestExpectation) {
        injectedService.perform { result in
            switch result {
    ...
    func testAwaitPerform() async {
        ...
        case .failure:
            XCTFail("Should not give an error")
        }
    }
}
